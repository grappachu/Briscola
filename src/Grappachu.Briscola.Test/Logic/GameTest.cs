...
namespace Grappachu.Briscola.Test.Logic
{
    public class GameTest
    {
        public GameTest()
        {
            ...
            _evaluatorMock = new Mock<IGameEvaluator>();
            _strategyMock = new Mock<IStrategy>();

            _players = new[] {"Io", "Davide"};

            _sut = new BriscolaGame(_deckFactoryMock.Object, _playerFactoryMock.Object, _evaluatorMock.Object);
        }
        ...
        private readonly Mock<IGameEvaluator> _evaluatorMock;
        ...
        {
            var p1Card = new Card("Coppe", 1);
            var p2Card = new Card("Bastoni", 5);
            ...
            // Ora mi diventa comodo usare una factory per creare i giocatori
            // anzich� passarli al costruttore come facevo in precedenza
            ...
            _sut.Start();
            _sut.State.Turn = 1; // Inizio arbitrariamente dal giocatore 2 per testare il ciclo

            _sut.PlayHand();

            _player1Mock.Verify(x => x.Play(It.Is<GameState>(s => s == _sut.State)));
            _player2Mock.Verify(x => x.Play(It.Is<GameState>(s => s == _sut.State)));
            _evaluatorMock.Verify(x => x.Assign(It.Is<GameState>(state => state == _sut.State)), Times.Once);
            _sut.State.Dish.Should().Have.SameSequenceAs(new[] {p2Card, p1Card});
        ...
        public void Start_cannot_be_called_when_players_are_1_or_more_than_4()
        {
            ...
            var g = new BriscolaGame(_deckFactoryMock.Object, _playerFactoryMock.Object, _evaluatorMock.Object);
